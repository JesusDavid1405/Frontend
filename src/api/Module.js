const URL = 'https://localhost:7279/api/Module/';
...
async function createModule(name, description) {
    try {
        ...
        if (response.ok) {
            ...
            displayGet.innerHTML='';
            getModule(); // Opcional: refresca la lista
        } else {
...
async function updateModule(id, name, description) {
    try{
        const response = await fetch(URL ,{
            method: 'PUT',
            headers: {
        ...
        if (response.ok) {
            ...
            ModalUpdate.hide();
            displayGet.innerHTML = '';
            getModule(); // volver a renderizar la lista
        } else {
