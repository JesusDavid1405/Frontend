const URL = 'https://localhost:7279/api/Rol/';
...
async function createRol(name, description) {
    try {
        const response = await fetch(URL, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            displayGet.innerHTML='';
            getRol(); // Opcional: refresca la lista
        } else {
...
async function updateRol(id, name, description) {
    try{
        const response = await fetch(URL ,{
            method: 'PUT',
            headers: {
        ...
        if (response.ok) {
            ...
            ModalUpdate.hide();
            displayGet.innerHTML = '';
            getRol(); // volver a renderizar la lista
        } else {
